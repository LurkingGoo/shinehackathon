...
| 8 | Regression from fix #4: Simulate button was `disabled` whenever the caseload carried an acute row — one press disabled it for the 30-min TTL | Button toggles: active incident ⇒ **"Reset demo"** → `POST /incidents/clear` (new endpoint + proxy rewrite) returns the caseload to calm; covered in `test_incident_persists_into_caseload` | **fixed 2026-07-02** |
...
`GET /incidents/stream` (SSE), `POST /incidents/simulate`,
`POST /incidents/clear` (demo reset), `GET /health`.
