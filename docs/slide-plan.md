...
|---|---|
| Overlapping text/images | Marp layouts are flow-based, so elements cannot overlap; content that doesn't fit is a visible overflow you fix by cutting words, not by nudging boxes |
| Theme inconsistency | One committed CSS theme (Warm Human tokens: terracotta primary, warm neutrals, humanist sans) applied to every slide by the compiler, so a slide *cannot* be off-theme |
| Stale designs | Deck source lives in `docs/slides/deck.md`; screenshots and diagrams are regenerated by script from the running product, so regenerate = current |
